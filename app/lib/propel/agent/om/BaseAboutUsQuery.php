...
/**
 * Base class that represents a query for the 'about_us' table.
 *
 *
 *
...
abstract class BaseAboutUsQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'travel', $modelName = 'AboutUs', $modelAlias = null)
    {
     ...
     * @param     AboutUsQuery|Criteria $criteria Optional Criteria to build the query from
     *
     * @return AboutUsQuery
     */
     ...
     *
     * @return   AboutUs|AboutUs[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = AboutUsPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return   AboutUs A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id` FROM `about_us` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new AboutUs();
            $obj->hydrate($row);
            AboutUsPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|AboutUs[]|mixed the list of results, formatted by the current formatter
     */
